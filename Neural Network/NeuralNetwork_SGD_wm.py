### Neural Network with Backpropagation Algorithm and mini-batch Stochastic Gradient Descent with Momentum
...
class NeuralNetwork:
    ...
    def cost(self, y, hy, theta1, theta2, ground_truth):
        ...
        firstterm=np.multiply(ground_truth, np.log(prob))
        
        reg=np.sum(np.power(theta1[:,1:],2))+np.sum(np.power(theta2[:,1:],2)) #regularization term (we are skipping the first term)
             
        total_cost=(1./np.shape(y)[0])*np.sum(firstterm) + self.C1*(1./(2*np.shape(y)[0]))*reg
    ...
    def backprop(self, a3, a2, a1, groundtruth, theta1, theta2, z2, mom1, mom2, gamma):
        ...
        mom1=gamma*mom1+self.alpha*Theta1Grad
        mom2=gamma*mom2+self.alpha*Theta2Grad

        theta1=theta1-mom1
        theta2=theta2-mom2

        
        return theta1, theta2, mom1, mom2
...
def runNN():
    ...
    ## setting gamma momentum between [0.5, 0.95]
    pom1=1./np.exp(-np.arange(0,Maxiter)/2500)
    gamma=0.5+(0.95-0.5)*(pom1-np.min(pom1))/(np.max(pom1)-np.min(pom1))
    ...
    for i in range(0, Maxiter):
        ...
        [theta1, theta2, mom1, mom2]=NN.backprop(hy, a2, a1, gd, theta1, theta2, z2, mom1, mom2, gamma[i])
